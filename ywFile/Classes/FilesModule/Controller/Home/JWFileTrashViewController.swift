//
//  JWFileTrashViewController.swift
...
class JWFileTrashViewController: JWFileBaseViewController {
    ...
    lazy var filesCollection: UICollectionView = {
        ...
        filesCollection.registerCellClassFromStrings(strings: ["JWFileDirCollectionViewCell", "JWFileTopSegmentCollectionViewCell", "JWFileTrashCollectionViewCell", "JWFileItemCollectionViewCell", "JWFileItemGridCollectionViewCell"])
    ...
    func loadData() {
        ...
        requestTrash(token: token)
    }
    
    
    func requestTrash(token:String) {
        
        let placeFiles: Array<Any> = [JWFilePlaceHolderModel(), "移至回收站后的文件在保存 30 天后将自动删除"]
        var allFiles: Array<Any>!
        requestFilesOfTrash(token: token) { (element) in
            allFiles = element
...
//MARK: request
extension JWFileTrashViewController {
    
    //MARK: 回收站file list
    func requestFilesOfTrash(token:String, completeClosure:@escaping(Array<Any>) -> Void) {
        
        var temp : Array<Any> = []
        SessionManager.request(api: JWFileModuleAPI.getTrashFile(
            token: token,
...
extension JWFileTrashViewController: JWFileListRefreshProtocol {
...
extension JWFileTrashViewController: JWFileListFlowLayoutDelegate {
...
extension JWFileTrashViewController: JWFileConfigureCollectionProtocol {
    ...
    func getReusedIndentifier(indexPath: IndexPath) -> String {
        ...
        if element is JWFilePlaceHolderModel {
            ...
            cell = "JWFileTrashCollectionViewCell"
        } else if element is JWFileModel {
            if (element as! JWFileModel).file_type == .Dir {
    ...
    func dispatchDataToCell(indexPath: IndexPath, data: Any?, cell: UICollectionViewCell) {
        ...
        if cell is JWFileTopSegmentCollectionViewCell {
            ...
            (cell as! JWFileTopSegmentCollectionViewCell).currentItemType = .Trash
        }
...
extension JWFileTrashViewController: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
